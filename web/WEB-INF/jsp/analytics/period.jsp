<%@page contentType="text/html" pageEncoding="UTF-8"%>
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Performance Analytics - ${period.name}</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/assets/styles.css" />
    <style>
        .score-bar { height: 8px; background: #e0e0e0; border-radius: 4px; overflow: hidden; }
        .score-fill { height: 100%; background: linear-gradient(90deg, #4CAF50, #8BC34A); transition: width 0.3s; }
        .badge { display: inline-block; padding: 2px 8px; border-radius: 12px; font-size: 12px; font-weight: bold; }
        .badge-elite { background: #FFD700; color: #333; }
        .badge-high { background: #4CAF50; color: white; }
        .badge-medium { background: #FF9800; color: white; }
        .rank-1 { color: #FFD700; font-weight: bold; }
        .rank-2 { color: #C0C0C0; font-weight: bold; }
        .rank-3 { color: #CD7F32; font-weight: bold; }
        .status-open { color: #4CAF50; font-weight: bold; }
        .status-closed { color: #FF9800; font-weight: bold; }
        .status-calculated { color: #2196F3; font-weight: bold; }
    </style>
</head>
<body>
<jsp:include page="../common/header.jsp" />

<h2 class="page-title">Performance Analytics - ${period.name}</h2>
<p class="page-sub">Period: ${period.startDate} - ${period.endDate}</p>

<div class="section-card">
    <h3 class="section-title compact-title">Period Details</h3>
    <table class="data-table">
        <tr><th>Period ID</th><td>${period.id}</td></tr>
        <tr><th>Name</th><td>${period.name}</td></tr>
        <tr><th>Start Date</th><td>${period.startDate}</td></tr>
        <tr><th>End Date</th><td>${period.endDate}</td></tr>
        <tr><th>Status</th><td class="status-${period.status.toLowerCase()}">${period.status}</td></tr>
        <tr><th>Created At</th><td>${period.createdAt}</td></tr>
        <c:if test="${not empty period.calculatedAt}">
            <tr><th>Calculated At</th><td>${period.calculatedAt}</td></tr>
        </c:if>
    </table>
</div>

<c:if test="${period.status == 'OPEN'}">
    <div class="section-card" style="background: #fff3cd; border: 1px solid #ffc107;">
        <p style="color: #856404; margin: 0;">
            <strong>Period is OPEN:</strong> Editorial board members can vote for mangaka performance. 
            <a href="${pageContext.request.contextPath}/main/analytics/vote/${period.id}" class="btn small">Go to Voting Page</a>
        </p>
    </div>
</c:if>

<c:if test="${period.status == 'CLOSED'}">
    <div class="section-card" style="background: #fff3cd; border: 1px solid #ffc107;">
        <p style="color: #856404; margin: 0;">
            <strong>Period is CLOSED:</strong> Voting is disabled. Admin can calculate performance to generate results.
        </p>
    </div>
</c:if>

<c:if test="${not empty results}">
    <div class="section-card">
        <h3 class="section-title compact-title">Overall Leaderboard</h3>
        <table class="data-table">
            <thead><tr><th>Rank</th><th>Mangaka</th><th>Popularity</th><th>Reliability</th><th>Quality</th><th>Overall</th><th>Badge</th></tr></thead>
            <tbody>
                <c:forEach items="${results}" var="result" varStatus="status">
                    <c:set var="rank" value="${status.index + 1}" />
                    <c:set var="mangakaName" value="${mangakaNames[result.mangakaId]}" />
                    <c:set var="popularityPercent" value="${result.popularityScore}" />
                    <c:set var="reliabilityPercent" value="${result.reliabilityScore}" />
                    <c:set var="qualityPercent" value="${result.qualityScore}" />
                    <c:set var="overallPercent" value="${result.overallScore}" />
                    <tr>
                        <td class="rank-${rank}">#${rank}</td>
                        <td>
                            <a href="${pageContext.request.contextPath}/main/analytics/mangaka/${result.mangakaId}?periodId=${period.id}">
                                <strong>${mangakaName}</strong>
                            </a>
                        </td>
                        <td>
                            <div class="score-bar"><div class="score-fill" style="width: ${popularityPercent}%"></div></div>
                            <small>${result.popularityScore}</small>
                        </td>
                        <td>
                            <div class="score-bar"><div class="score-fill" style="width: ${reliabilityPercent}%"></div></div>
                            <small>${result.reliabilityScore}</small>
                        </td>
                        <td>
                            <div class="score-bar"><div class="score-fill" style="width: ${qualityPercent}%"></div></div>
                            <small>${result.qualityScore}</small>
                        </td>
                        <td><strong>${result.overallScore}</strong></td>
                        <td>
                            <c:if test="${rank == 1}"><span class="badge badge-elite">Elite</span></c:if>
                            <c:if test="${rank == 2 || rank == 3}"><span class="badge badge-high">Top Tier</span></c:if>
                            <c:if test="${rank > 3 && rank <= 10}"><span class="badge badge-medium">High</span></c:if>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
    </div>
</c:if>

<c:if test="${empty results && period.status == 'CALCULATED'}">
    <div class="section-card">
        <p class="muted">No performance results available for this period.</p>
    </div>
</c:if>

<c:if test="${empty results && period.status != 'CALCULATED'}">
    <div class="section-card">
        <p class="muted">Results will be available after the period is calculated.</p>
    </div>
</c:if>

<div class="section-card">
    <h3 class="section-title compact-title">Quick Links</h3>
    <div style="display: flex; gap: 8px; flex-wrap: wrap;">
        <a class="btn" href="${pageContext.request.contextPath}/main/analytics">Back to Analytics</a>
        <a class="btn" href="${pageContext.request.contextPath}/main/analytics/popular?periodId=${period.id}">Top Popular</a>
        <a class="btn" href="${pageContext.request.contextPath}/main/analytics/reliable?periodId=${period.id}">Most Reliable</a>
        <a class="btn" href="${pageContext.request.contextPath}/main/analytics/quality?periodId=${period.id}">Highest Quality</a>
    </div>
</div>

<jsp:include page="../common/footer.jsp" />
</body>
</html>
