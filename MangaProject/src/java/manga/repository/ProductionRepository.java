...
public class ProductionRepository {
    ...
    public List<TaskSummary> listTasksByAssistant(long assistantId) {
        return queryTasks(Long.valueOf(assistantId));
    }

    private List<TaskSummary> queryTasks(Long assistantId) {
        String sql =
            ...
            + (assistantId == null ? "" : "WHERE t.assistantId = ? ")
        ...
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            if (assistantId != null) {
                ps.setLong(1, assistantId.longValue());
            }
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
